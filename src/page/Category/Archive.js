...
import {BackHandler, FlatList, Modal, ScrollView, Text, View,Switch,I18nManager} from 'react-native';
            ...
            <View style={{backgroundColor:Config.background,flex:1}}>
                ...
                <Modal onRequestClose={()=>{this.setState({filter_modal:false})}} animationType={'slide'} visible={this.state.filter_modal}>
                    <View style={{flex:1,padding:10}}>
                        <ScrollView>
                            <ButtonGroup selectedIndex={this.state.filter_price} buttons={[lan.Paid,lan.Free,lan.All]} onPress={(index)=>{this.setState({filter_price:index})}}/>
                            <View style={{height:15}}/>
                            <ButtonGroup selectedIndex={this.state.filter_type} buttons={[lan.Single,lan.Webinar,lan.Course,lan.All]} onPress={(index)=>{this.setState({filter_type:index})}}/>
                            <View style={{height:13}}/>
                            <View style={{flex:1,flexDirection:'row',padding:12}}>
                                <View style={{flex:1}}><Text>{lan.Physical}</Text></View>
                            ...
                            <View style={{height:13}}/>
                            <View style={{flex:1,flexDirection:'row',padding:12}}>
                                <View style={{flex:1}}><Text>{lan.Support}</Text></View>
                            ...
                            <View style={{height:13}}/>
                            <View style={{flex:1,flexDirection:'row',padding:12}}>
                                <View style={{flex:1}}><Text>{lan.filter_discount}</Text></View>
                        ...
                        <Button title={lan.Filter} onPress={()=>{this.refreshData();this.setState({filter_modal:false});}} style={{position:'absolute',bottom:0,height:30}}/>
                    </View>
                </Modal>
            </View>
        );
