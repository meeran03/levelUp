...
export default class ProductView extends React.PureComponent{
                                ...
                                <Card containerStyle={style.quizContainer}>
                                    ...
                                    <View style={{flex:1,flexDirection:'row',marginTop:30}}>
                                        ...
                                        <View style={{flex:1,flexDirection:'row',alignItems:'center',justifyContent:'center'}}>
                                            ...
                                            <Text style={{color:'#CDCDCD'}}> {item.time} {lan.Minutes}</Text>
                                        </View>
                    ...
                    renderItem={({item, index})=>{
                        return(
                            <Card containerStyle={style.quizContainer}>
                                ...
                                <Button onPress={()=>{this._openUrl(item.certificate)}} containerStyle={{width:120,position:'absolute',right:0,top:10}} title={lan.download} buttonStyle={{backgroundColor:'#13CE9C',borderRadius:25}}/>
                            </Card>
                        )
                    }}
                />
        ...
        if((this.state.tabIndex == 0 || this.state.tabIndex == 1) && this.state.product.buy == '0'){
            return (
                <View style={{position:'absolute',height:60, width:'100%',bottom:0,backgroundColor:'#fff',elevation:10}}>
                    <Button title={lan.enroll_on_the_course} containerStyle={{width:'80%',alignSelf:'center',paddingTop:10}} buttonStyle={{backgroundColor:'#13CE9C',borderRadius:25}} onPress={()=>{this._openBuyModal()}}/>
                </View>
            )
        }
        if(this.state.tabIndex == 2){
            return (
                <View style={{position:'absolute',height:60, width:'100%',bottom:0,backgroundColor:'#fff',elevation:10}}>
                    <Button title={lan.write_a_review} onPress={()=>{this._openCommentModal()}} containerStyle={{width:'80%',alignSelf:'center',paddingTop:10}} buttonStyle={{backgroundColor:'#13CE9C',borderRadius:25}}/>
                </View>
            )
        }
        if(this.state.tabIndex == 3){
            return (
                <View style={{position:'absolute',height:60, width:'100%',bottom:0,backgroundColor:'#fff',elevation:10}}>
                    {this.state.product.support == '1'?
                        ...
                        :
                        <Button title={lan.support_is_disabled} containerStyle={{width:'80%',alignSelf:'center',paddingTop:10}} buttonStyle={{backgroundColor:'gray',borderRadius:25}}/>
                    }
                </View>
            )
        }
    ...
    _tabsArray = () =>{
        ...
        if(this.state.product['quizzes_enable'] == true && this.state.product['certificates_enable']){
            return [
                this.state.product['type']=="webinar"?lan.live:lan.Courses,
                lan.Information,
                lan.Comments,
                lan.support,
                lan.Quizzes,
                ...
                <View style={{flex:1, backgroundColor: Config.background}}>
                    ...
                    <ScrollView>
                        ...
                        <SegmentedControlTab
                            ...
                            activeTabStyle={{borderBottomWidth:2,backgroundColor:'transparent',borderBottomColor:Config.customColor}}
                            activeTabTextStyle={{color:Config.secondaryColor}}
                    ...
                    <Modal onRequestClose={()=>{this.setState({modalBuy:false})}} visible={this.state.modalBuy}>
                        <View style={{flex:1, backgroundColor: Config.background}}>
                            <Header
                                containerStyle={{height:60}}
                                backgroundColor={Config.customColor}
                                leftComponent={<Icon name='ios-arrow-back' color='black' type='ionicon' onPress={()=>{this.setState({modalBuy:false})}} />}
                                leftContainerStyle={{bottom:14,left:10}}
                                centerComponent={{text:'Enroll on the course',numberOfLines:1,style:{color:'black',fontFamily:'robotobold'}}}
